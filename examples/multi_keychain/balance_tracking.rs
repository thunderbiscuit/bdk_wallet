...
fn main() {
    ...
    let savings_balance = wallet.balance_keychain(AccountType::Savings);
    println!("   Savings account:");
    println!("   - Total: {} sats", savings_balance.total());

    // Balance for spending
    let spending_balance = wallet.balance_keychain(AccountType::Spending);
    println!("   Spending account:");
    println!("   - Total: {} sats", spending_balance.total());

    // Balance for donations
    let donations_balance = wallet.balance_keychain(AccountType::Donations);
    println!("   Donations account:");
    println!("   - Total: {} sats\n", donations_balance.total());

    // TODO PR #318: Bring this back as a range on the method if we like the API
