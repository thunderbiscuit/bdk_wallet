...
{
    /// Create a new `Wallet` given the `KeyRing`, with default lookahead and no spk cache.
    ///
    /// Note: For this method the network used in `KeyRing` must be the mainnet or one of the test
    /// networks.
    pub fn new(mut keyring: KeyRing<K>) -> Self {
        ...
        for (keychain, desc) in descriptors {
            let _inserted = index
                ...
                .expect("descriptor has been checked inside the keyring already");
            assert!(_inserted);
        }

        let keychain_tx_graph = KeychainTxGraph::new(index);
    ...
    //     /// with single descriptor wallets, for example setting a [`change_policy`] on
    // [`TxBuilder`]     /// and related methods such as [`do_not_spend_change`]. This is
    // because all payments are     /// received on the external keychain (including change),
    // and without a change keychain     /// BDK lacks enough information to distinguish between
    // change and outside payments.     ///
    //     /// Additionally because this wallet has no internal (change) keychain, all methods that
    //     /// require a [`KeychainKind`] as input, e.g. [`reveal_next_address`] should only be
    // called     /// using the [`External`] variant. In most cases passing [`Internal`] is
    // treated as the     /// equivalent of [`External`] but this behavior must not be relied
    // on.     ///
    //     /// # Example
    //     ///
    //     /// ```rust
    ...
    //     /// using the existing receive and change wallet creation logic. Note that you can only
    // use this     /// method with public extended keys (`xpub` prefix) to create watch-only
    // wallets.     ///
    ...
    //     ///
    //     /// # Errors
    //     /// Returns an error if the descriptor is invalid, not a 2-path multipath descriptor, or
    // if     /// the descriptor provided contains an extended private key (`xprv` prefix).
    //     ///
    //     /// # Synopsis
    //     ///
    //     /// ```rust
    ...
    //     ///     .unwrap();
    //     ///
    //     /// // The multipath descriptor automatically creates separate receive and change
    // descriptors     /// let receive_addr = wallet.peek_address(KeychainKind::External, 0);
    // // Uses path /0/*     /// let change_addr = wallet.peek_address(KeychainKind::Internal,
    // 0);   // Uses path /1/*     /// assert_ne!(receive_addr.address, change_addr.address);
    //     /// ```
    //     ///
    ...
    //     /// # Panics
    //     ///
    //     /// This panics when the caller requests for an address of derivation index greater than
    // the     /// [BIP32](https://github.com/bitcoin/bips/blob/master/bip-0032.mediawiki) max index.
    ...
    //         AddressInfo {
    //             index,
    //             address: Address::from_script(&spk, self.network).expect("must have address
    // form"),             keychain,
    //         }
    //     }
    ...
    //     ///
    //     /// This will increment the keychain's derivation index. If the keychain's descriptor
    // doesn't     /// contain a wildcard or every address is already revealed up to the maximum
    // derivation     /// index defined in [BIP32](https://github.com/bitcoin/bips/blob/master/bip-0032.mediawiki),
    //     /// then the last revealed address will be returned.
    //     ///
    //     /// **WARNING**: To avoid address reuse you must persist the changes resulting from one
    // or more     /// calls to this method before closing the wallet. For example:
    //     ///
    //     /// ```rust,no_run
    ...
    //     ///
    //     /// **WARNING**: To avoid address reuse you must persist the changes resulting from one
    // or more     /// calls to this method before closing the wallet. See
    // [`Wallet::reveal_next_address`].     pub fn reveal_addresses_to(
    //         &mut self,
    //         keychain: KeychainKind,
    //         index: u32,
    ...
    //     /// Get the next unused address for the given `keychain`, i.e. the address with the
    // lowest     /// derivation index that hasn't been used in a transaction.
    //     ///
    ...
    //     /// **WARNING**: To avoid address reuse you must persist the changes resulting from one
    // or more     /// calls to this method before closing the wallet. See
    // [`Wallet::reveal_next_address`].     pub fn next_unused_address(&mut self, keychain:
    // KeychainKind) -> AddressInfo {         let keychain = self.map_keychain(keychain);
    //         let index = &mut self.indexed_graph.index;
    ...
    // given     /// `index` was actually used, i.e. the wallet has previously indexed a tx output
    // for the     /// derived spk.
    //     ///
    ...
    //     /// List all relevant outputs (includes both spent and unspent, confirmed and
    // unconfirmed).     ///
    ...
    // *stop gap*.     ///
    //     /// Note carefully that iterators go over **all** script pubkeys on the keychains (not
    // what     /// script pubkeys the wallet is storing internally).
    ...
    //     ///
    //     /// This is used for providing a previous output's value so that we can use
    // [`calculate_fee`]     /// or [`calculate_fee_rate`] on a given transaction. Outputs
    // inserted with this method will     /// not be returned in [`list_unspent`] or
    // [`list_output`].     ///
    //     /// **WARNINGS:** This should only be used to add `TxOut`s that the wallet does not own.
    // Only     /// insert `TxOut`s that you trust the values for!
    //     ///
    //     /// You must persist the changes resulting from one or more calls to this method if you
    // need     /// the inserted `TxOut` data to be reloaded after closing the wallet.
    ...
    //     /// Calculates the fee of a given transaction. Returns [`Amount::ZERO`] if `tx` is a
    // coinbase     /// transaction.
    //     ///
    //     /// To calculate the fee for a [`Transaction`] with inputs not owned by this wallet you
    // must     /// manually insert the TxOut(s) into the tx graph using the [`insert_txout`]
    // function.     ///
    ...
    // you     /// must manually insert the TxOut(s) into the tx graph using the
    // [`insert_txout`] function.     ///
    //     /// Note `tx` does not have to be in the graph for this to work.
    //     ///
    //     /// # Examples
    //     ///
    ...
    //     /// * The [`ChainPosition`] of the transaction in the best chain - whether the
    // transaction is     ///   confirmed or unconfirmed. If the transaction is confirmed, the
    // anchor which proves the     ///   confirmation is provided. If the transaction is
    // unconfirmed, the unix timestamp of when     ///   the transaction was last seen in the
    // mempool is provided.     ///
    //     /// ```rust, no_run
    ...
    //     ///         transitively: None,
    //     ///     } => println!(
    //     ///         "tx is confirmed at height {}, we know this since {}:{} is in the best
    // chain",     ///         anchor.block_id.height, anchor.block_id.height,
    ...
    //     ///
    //     /// A transaction is relevant when it spends from or spends to at least one tracked
    // output. A     /// transaction is canonical when it is confirmed in the best chain, or
    // does not conflict     /// with any transaction confirmed in the best chain.
    //     ///
    //     /// To iterate over all transactions, including those that are irrelevant and not
    // canonical, use     /// [`TxGraph::full_txs`].
    //     ///
    ...
    //     /// function.
    //     ///
    //     /// This is a helper method equivalent to collecting the result of
    // [`Wallet::transactions`]     /// into a [`Vec`] and then sorting it.
    //     ///
    //     /// # Example
    //     ///
    //     /// ```rust,no_run
    ...
    //     /// # let mut wallet:Wallet = todo!();
    //     /// // Transactions by chain position: first unconfirmed then descending by confirmed
    // height.     /// let sorted_txs: Vec<WalletTx> =
    //     ///     wallet.transactions_sort_by(|tx1, tx2|
    // tx2.chain_position.cmp(&tx1.chain_position));     /// # Ok::<(), anyhow::Error>(())
    //     /// ```
    ...
    // let psbt = {     ///    let mut builder =  wallet.build_tx();
    //     ///    builder
    ...
    //         let external_descriptor = keychains.get(&KeychainKind::External).expect("must
    // exist");         let internal_descriptor = keychains.get(&KeychainKind::Internal);
    ...
    //             // If they didn't tell us the current height, we assume it's the latest sync
    // height.             None => {
    ...
    //                 }
    //             }
    //             // Specific nLockTime required and we have no constraints, so just set to that
    // value             Some(x) if requirements.timelock.is_none() => x,
    ...
    //     ///
    //     /// ```no_run
    //     /// # // TODO: remove norun -- bumping fee seems to need the tx in the wallet database
    // first.     /// # use std::str::FromStr;
    //     /// # use bitcoin::*;
    //     /// # use bdk_wallet::*;
    //     /// # use bdk_wallet::ChangeSet;
    //     /// # use bdk_wallet::error::CreateTxError;
    ...
    //     /// # let to_address =
    // Address::from_str("2N4eQYCbKUHCCTUjBJeHcJp9ok6J2GZsTDt").unwrap().assume_checked();     ///
    // let mut psbt = {     ///     let mut builder = wallet.build_tx();
    //     ///     builder
    //     ///         .add_recipient(to_address.script_pubkey(), Amount::from_sat(50_000));
    //     ///     builder.finish()?
    //     /// };
    ...
    // .cloned()?;                         Ok((prev_tx, prev_txout, chain_position))
    //                     })
    ...
    // signer's     /// [`SignerOrdering`]. This function returns the `Result` type with an
    // encapsulated `bool` that     /// has the value true if the PSBT was finalized, or false
    // otherwise.     ///
    //     /// The [`SignOptions`] can be used to tweak the behavior of the software signers, and
    // the way     /// the transaction is finalized at the end. Note that it can't be guaranteed
    // that *every*     /// signers will follow the options, but the "software signers" (WIF
    // keys and `xprv`) defined     /// in this library will.
    //     ///
    //     /// ## Example
    //     ///
    //     /// ```
    ...
    //     /// # Ok::<(),anyhow::Error>(())
    //     pub fn sign(&self, psbt: &mut Psbt, sign_options: SignOptions) -> Result<bool,
    // SignerError> {         // This adds all the PSBT metadata for the inputs, which will help
    // us later figure out how         // to derive our keys
    ...
    //         // If the user hasn't explicitly opted-in, refuse to sign the transaction unless
    // every input         // is using `SIGHASH_ALL` or `SIGHASH_DEFAULT` for taproot
    ...
    //     ///
    //     /// It's the "public" version of the wallet's descriptor, meaning a new descriptor that
    // has     /// the same structure but with the all secret keys replaced by their
    // corresponding public key.     /// This can be used to build a watch-only version of a
    // wallet.     pub fn public_descriptor(&self, keychain: KeychainKind) ->
    // &ExtendedDescriptor {         self.indexed_graph
    //             .index
    //             .get_descriptor(self.map_keychain(keychain))
    //             .expect("keychain must exist")
    //     }

    //     /// Finalize a PSBT, i.e., for each input determine if sufficient data is available to
    // pass     /// validation and construct the respective `scriptSig` or `scriptWitness`.
    // Please refer to     /// [BIP174](https://github.com/bitcoin/bips/blob/master/bip-0174.mediawiki#Input_Finalizer),
    ...
    //             // This is for a small performance gain. Although `.filter` filters out excess
    // txs, it             // will still consume the internal `CanonicalIter` entirely. Having a
    // `.take` here             // allows us to stop further unnecessary canonicalization.
    //             .take(prev_txids.len())
    //             .map(|canon_tx| {
    //                 let txid = canon_tx.tx_node.txid;
    //                 match canon_tx.chain_position {
    //                     ChainPosition::Confirmed { anchor, .. } => (txid,
    // anchor.block_id.height),                     ChainPosition::Unconfirmed { .. } => (txid,
    // u32::MAX),                 }
    //             })
    ...
    //             if psbt_input.final_script_sig.is_some() ||
    // psbt_input.final_script_witness.is_some() {                 continue;
    //             }
    ...
    //             // - Try to derive the descriptor by looking at the txout. If it's in our
    // database, we             //   know exactly which `keychain` to use, and which derivation
    // index it is             // - If that fails, try to derive it by looking at the psbt
    // input: the complete logic is             //   in `src/descriptor/mod.rs`, but it will
    // basically look at `bip32_derivation`,             //   `redeem_script` and
    // `witness_script` to determine the right derivation             // - If that also fails,
    // it will try it on the internal descriptor, if present             let desc = psbt
    //                 .get_utxo_for(n)
    //                 .and_then(|txout| self.get_descriptor_for_txout(&txout))
    //                 .or_else(|| {
    //                     self.indexed_graph.index.keychains().find_map(|(_, desc)| {
    //                         desc.derive_from_psbt_input(psbt_input, psbt.get_utxo_for(n),
    // &self.secp)                     })
    //                 });
    ...
    //     /// The index of the next address that you would get if you were to ask the wallet for a
    // new     /// address
    ...
    //     /// Informs the wallet that you no longer intend to broadcast a tx that was built from
    // it.     ///
    ...
    // txout_index.index_of_spk(txout.script_pubkey.clone()) {                 // NOTE: unmark_used
    // will **not** make something unused if it has actually been used                 // by a
    // tx in the tracker. It only removes the superficial marking.
    ...
    //                 // NOTE: the UTxOs returned by the following method already belong to wallet
    // as the                 // call chain uses get_tx_node infallibly
    //                 .filter_chain_unspents(
    //                     &self.chain,
    //                     self.chain.tip().block_id(),
    //                     CanonicalizationParams::default(),
    ...
    //                 })
    //                 // only process UTXOs not selected manually, they will be considered later in
    // the                 // chain
    //                 // NOTE: this avoid UTXOs in both required and optional list
    //                 .filter(|may_spend|
    // !manually_selected_outpoints.contains(&may_spend.outpoint))                 // only add
    // to optional UTxOs those which satisfy the change policy if we reuse                 //
    // change                 .filter(|local_output| {
    ...
    //                 Utxo::Local(utxo) => {
    //                     *psbt_input =
    //                         match self.get_psbt_input(utxo, params.sighash,
    // params.only_witness_utxo) {                             Ok(psbt_input) => psbt_input,
    //                             Err(e) => match e {
    ...
    //         // Try to find the prev_script in our db to figure out if this is internal or
    // external,         // and the derivation index
    //         let &(keychain, child) = self
    //             .indexed_graph
    //             .index
    ...
    // {         // We need to borrow `psbt` mutably within the loops, so we have to allocate a
    // vec for all         // the input utxos and outputs
    ...
    // of how     /// to persist staged wallet changes see [`Wallet::reveal_next_address`].
    //     pub fn apply_update(&mut self, update: impl Into<Update>) -> Result<(),
    // CannotConnectError> {         let update = update.into();
    ...
    //     /// Get a mutable reference of the staged [`ChangeSet`] that is yet to be committed (if
    // any).     pub fn staged_mut(&mut self) -> Option<&mut ChangeSet> {
    //         if self.stage.is_empty() {
    //             None
    //         } else {
    //             Some(&mut self.stage)
    ...
    //     pub fn apply_block(&mut self, block: &Block, height: u32) -> Result<(),
    // CannotConnectError> {         let connected_to = match height.checked_sub(1) {
    ...
    //                     unreachable!("connected_to is derived from the block so must be
    // consistent")                 }
    ...
    //     ///
    //     /// The `connected_to` parameter informs the wallet how this block connects to the
    // internal     /// [`LocalChain`]. Relevant transactions are filtered from the `block` and
    // inserted into the     /// internal [`TxGraph`].
    //     ///
    //     /// **WARNING**: You must persist the changes resulting from one or more calls to this
    // method     /// if you need the inserted block data to be reloaded after closing the
    // wallet.     /// See [`Wallet::reveal_next_address`].
    ...
    //     ///
    //     /// This method takes in an iterator of `(tx, last_seen)` where `last_seen` is the
    // timestamp of     /// when the transaction was last seen in the mempool. This is used for
    // conflict resolution     /// when there is conflicting unconfirmed transactions. The
    // transaction with the later     /// `last_seen` is prioritized.
    //     ///
    //     /// **WARNING**: You must persist the changes resulting from one or more calls to this
    // method     /// if you need the applied unconfirmed transactions to be reloaded after
    // closing the wallet.     /// See [`Wallet::reveal_next_address`].
    ...
    // mempool.     /// Eviction means that these transactions are not considered canonical by
    // default, and will     /// no longer be part of the wallet's [`transactions`] set. This
    // can happen for example when     /// a transaction is dropped from the mempool due to low
    // fees or conflicts with another     /// transaction.
    //     ///
    ...
    // evicted     /// transaction can become canonical again if it is later observed on-chain or
    // seen in the     /// mempool with a higher priority (e.g., due to a fee bump).
    //     ///
    //     /// ## Parameters
    //     ///
    //     /// `evicted_txs`: An iterator of `(Txid, u64)` tuples, where:
    //     /// - `Txid`: The transaction ID of the transaction to be evicted.
    //     /// - `u64`: The timestamp indicating when the transaction was evicted from the mempool.
    // This     ///   will usually correspond to the time of the latest chain sync. See docs for
    //     ///   [`start_sync_with_revealed_spks`].
    //     ///
    //     /// ## Notes
    //     ///
    //     /// - Not all blockchain backends support automatic mempool eviction handling - this
    // method may     ///   be used in such cases. It can also be used to negate the effect of
    //     ///   [`apply_unconfirmed_txs`] for a particular transaction without the need for an
    // additional     ///   sync.
    //     /// - The changes are staged in the wallet's internal state and must be persisted to
    // ensure they     ///   are retained across wallet restarts. Use [`Wallet::take_staged`] to
    // retrieve the staged     ///   changes and persist them to your database of choice.
    //     /// - Evicted transactions are removed from the wallet's canonical transaction set, but
    // the data     ///   remains in the wallet's internal transaction graph for historical
    // purposes.     /// - Ensure that the timestamps provided are accurate and monotonically
    // increasing, as they     ///   influence the wallet's canonicalization logic.
    //     ///
    ...
    //     /// [`SyncRequest`] collects all revealed script pubkeys from the wallet keychain needed
    // to     /// start a blockchain sync with a spk based blockchain client.
    //     ///
    ...
    //     /// [`FullScanRequest] collects iterators for the wallet's keychain script pub keys
    // needed to     /// start a blockchain full scan with a spk based blockchain client.
    //     ///
    ...
    //     ///
    //     /// The time of the scan is the current system time and is used to record the tx
    // last-seen for     /// mempool transactions. To supply your own start time see
