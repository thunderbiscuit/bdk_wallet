...
#[allow(unused)]
type IndexedTxOut<K = KeychainKind> = ((K, u32), FullTxOut<ConfirmationBlockTime>);
...
#[derive(Debug)]
pub struct Wallet<K: Ord = KeychainKind> {
    chain: LocalChain,
...
#[derive(Debug, Clone)]
pub struct Update<K: Ord = KeychainKind> {
    ...
    K: Ord + Clone + core::fmt::Debug,
{
    ...
    /// let keyring = KeyRing::standard(Network::Testnet, EXTERNAL, INTERNAL)?;
...
macro_rules! doctest_wallet {
    () => {{
        ...
        let mut wallet = Wallet::create(KeyRing::standard(Network::Regtest, descriptor, change_descriptor).expect("valid descriptors"))
            .create_wallet_no_persist();
...
mod test {
    ...
    fn test_create_two_path_wallet() {
        ...
        // Test successful creation of a two-path keyring
        let keyring = KeyRing::from_two_path_descriptor(Network::Testnet, two_path_descriptor);
        assert!(keyring.is_ok());

        let wallet = Wallet::create(keyring.unwrap()).create_wallet_no_persist();
    ...
    fn test_create_two_path_wallet_invalid_descriptor() {
        ...
        let keyring = KeyRing::from_two_path_descriptor(Network::Testnet, single_path_descriptor);
        assert!(matches!(
            keyring,
            Err(KeyRingError::Descriptor(DescriptorError::MultiPath))
        ));
        ...
        let keyring =
            KeyRing::from_two_path_descriptor(Network::Testnet, private_multipath_descriptor);
        assert!(matches!(
            keyring,
            Err(KeyRingError::Descriptor(DescriptorError::Miniscript(
                Unexpected(..)
            )))
        ));
        ...
        let keyring = KeyRing::from_two_path_descriptor(Network::Testnet, three_path_descriptor);
        assert!(matches!(
            keyring,
            Err(KeyRingError::Descriptor(DescriptorError::MultiPath))
        ));
        ...
        let keyring = KeyRing::from_two_path_descriptor(Network::Testnet, invalid_descriptor);
