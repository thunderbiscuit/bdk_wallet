...
//         // For UTXOs that doesn't exist in DB (Utxo::Foreign), they will have lowest priority to
// be         // selected
//         let utxos = {
...
// pub fn decide_change(remaining_amount: Amount, fee_rate: FeeRate, drain_script: &Script) ->
// Excess {     // drain_output_len = size(len(script_pubkey)) + len(script_pubkey) +
// size(output_value)     let drain_output_len = serialize(drain_script).len() + 8usize;
//     let change_fee =
...
//                     **fee_amount += fee_rate
//                         * TxIn::default() .segwit_weight()
//                           .checked_add(weighted_utxo.satisfaction_weight) .expect("`Weight`
//                           addition should not cause an integer overflow");
//                     **selected_amount += weighted_utxo.utxo.txout().value;
//                     Some(weighted_utxo.utxo)
//                 } else {
//                     None
...
//         let fee = fee_rate
//             * TxIn::default() .segwit_weight() .checked_add(weighted_utxo.satisfaction_weight)
//               .expect("`Weight` addition should not cause an integer overflow");
...
// #[derive(Debug)]
// enum BnbError {
//     /// Branch and bound coin selection tries to avoid needing a change by finding the right
// inputs     /// for the desired outputs plus fee, if there is not such combination this error is
// thrown     NoExactMatch,
//     /// Branch and bound coin selection possible attempts with sufficiently big UTXO set could
...
//     fn default() -> Self {
//         Self {
//             // P2WPKH cost of change -> value (8 bytes) + script len (1 bytes) + script (22
// bytes)             size_of_change: 8 + 1 + 22,
...
//         // be negative as well.
//         //
//         // If the sum of curr_value and curr_available_value is negative or lower than our
// target,         // we can immediately exit with an error, as it's guaranteed we will never find a
// solution         // if we actually run the BnB.
...
//                 let (utxo_fees, utxo_value) =
// required_ogs.iter().chain(optional_ogs.iter()).fold(                     (Amount::ZERO,
// Amount::ZERO),                     |(mut fees, mut value), utxo| {
//                         fees += utxo.fee;
...
// curr_available_value,             // or the selected value is out of range.
...
//                 backtrack = true;
//
//                 // If we found a solution better than the previous one, or if there wasn't
// previous                 // solution, update the best solution
...
//                     curr_available_value +=
// optional_utxos[current_selection.len()].effective_value;                 }
//
...
//         rand: &mut R,
//     ) -> Result<CoinSelectionResult, InsufficientFunds> {
//         // We put the required UTXOs first and then the randomize optional UTXOs to take as
// needed         let utxos = {
...
//                 if needed == Amount::from_sat(300_254) && available ==
// Amount::from_sat(300_010)));     }
//
//     #[test]
...
//         let fee_rate = FeeRate::from_sat_per_vb_unchecked(10);
//         let utxos: Vec<OutputGroup> = generate_same_value_utxos(Amount::from_sat(100_000),
// 100_000)             .into_iter()
//             .map(|u| OutputGroup::new(u, fee_rate))
//             .collect();
//
