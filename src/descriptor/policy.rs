...
// //! Descriptor policy
// //!
// //! This module implements the logic to extract and represent the spending policies of a
// descriptor //! in a more human-readable format.
// //!
...
// //! let desc =
// "wsh(and_v(v:pk(cV3oCth6zxZ1UVsHLnGothsWNsaoxRhC6aeNi5VbSdFpwUkgkEci),
// or_d(pk(cVMTy7uebJgvFaSBwcgvwk8qn8xSLc97dKow4MBetjrrahZoimm2),older(12960))))"; //!
...
//         calc_checksum(&serde_json::to_string(self).expect("Failed to serialize a
// SatisfiableItem"))             .expect("Failed to compute a SatisfiableItem id")
//     }
// }
...
//                 // `indexes` at this point is a Vec<Vec<usize>>, with the "n choose k" of items
// (m                 // of n)
//                 indexes
//                     .into_iter()
//                     // .inspect(|x| println!("--- orig --- {:?}", x))
//                     // we map each of the combinations of elements into a tuple of ([chosen
// items],                     // [conditions]). unfortunately, those items have potentially more
// than one                     // condition (think about ORs), so we also use `mix` to expand
// those, i.e. [[0],                     // [1, 2]] becomes [[0, 1], [0, 2]]. This is necessary to
// make sure that we                     // consider every possible options and check whether or not
// they are compatible.                     // since this step can turn one item of the iterator
// into multiple ones, we use                     // `flat_map()` to expand them out
//                     .flat_map(|i_vec| {
//                         mix(i_vec
//                             .iter()
//                             .map(|i| {
...
//     /// Can not merge CSV or timelock values unless both are less than or both are equal or
// greater     /// than 500_000_000
//     MixedTimelockUnits,
...
//         // if items.len() == threshold, selected can be omitted and we take all of them by
// default         let default = match &self.item {
...
//                 // if we have something, make sure we have enough items. note that the user can
// set                 // an empty value for this step in case of n-of-n, because `selected` is set
// to all                 // the elements above
...
//     // prefix) even if we are in a taproot descriptor. We just want some kind of unique
// identifier     // for a key, so it doesn't really matter how the identifier is computed.
//     match key {
//         DescriptorPublicKey::Single(SinglePub {
//             key: SinglePubKey::FullKey(pk),
//             ..
...
//                 Some(policy)
//             }
//             Terminal::Sha256(hash) => Some(SatisfiableItem::Sha256Preimage { hash: *hash
// }.into()),             Terminal::Hash256(hash) => {
...
//         /// CSV should consider different inputs, but we consider the worst condition for the tx
// as         /// whole
...
//                 ShInner::SortedMulti(ref keys) => make_sortedmulti(keys, signers, build_sat,
// secp),                 ShInner::Wsh(wsh) => match wsh.as_inner() {
...
//                 WshInner::Ms(ms) => Ok(ms.extract_policy(signers, build_sat, secp)?),
//                 WshInner::SortedMulti(ref keys) => make_sortedmulti(keys, signers, build_sat,
// secp),             },
...
//     const TPRV0_STR:&str =
// "tprv8ZgxMBicQKsPdZXrcHNLf5JAJWFAoJ2TrstMRdSKtEggz6PddbuSkvHKM9oKJyFgZV1B7rw8oChspxyYbtmEXYyg1AjfWbL3ho3XHDpHRZf"
// ;     const TPRV1_STR:&str =
...
//         assert_matches!(&policy.item, EcdsaSignature(PkOrF::Fingerprint(f)) if f ==
// &fingerprint);         assert_matches!(&policy.contribution, Satisfaction::None);
...
//         assert_matches!(&policy.item, EcdsaSignature(PkOrF::Fingerprint(f)) if f ==
// &fingerprint);         assert_matches!(&policy.contribution, Satisfaction::Complete {condition}
// if condition.csv.is_none() && condition.timelock.is_none());     }
...
//         assert_matches!(&policy.item, Thresh { items, threshold } if items.len() == 3 &&
// threshold == &2);

//         assert_matches!(&policy.contribution, Satisfaction::PartialComplete { n, m, items,
// conditions, .. } if n == &3              && m == &2
//              && items.len() == 3
//              && conditions.get(&vec![0,1]).unwrap().iter().next().unwrap().csv.is_none()
//              && conditions.get(&vec![0,2]).unwrap().iter().next().unwrap().csv ==
...
//         let secp = Secp256k1::new();
//         let (prvkey0, _pubkey0, _fingerprint0) = setup_keys(TPRV0_STR, PATH, &secp);
//         let locktime_threshold = 500000000; // if less than this means block number, else block
// time in seconds         let locktime_blocks = 100;
...
//     const ALICE_TPRV_STR:&str =
// "tprv8ZgxMBicQKsPf6T5X327efHnvJDr45Xnb8W4JifNWtEoqXu9MRYS4v1oYe6DFcMVETxy5w3bqpubYRqvcVTqovG1LifFcVUuJcbwJwrhYzP"
// ;     const BOB_TPRV_STR:&str =
// "tprv8ZgxMBicQKsPeinZ155cJAn117KYhbaN6MV3WeG6sWhxWzcvX1eg1awd4C9GpUN1ncLEM2rzEvunAg3GizdZD4QPPCkisTz99tXXB4wZArp"
// ;     const CAROL_TPRV_STR:&str =
// "tprv8ZgxMBicQKsPdC3CicFifuLCEyVVdXVUNYorxUWj3iGZ6nimnLAYAY9SYB7ib8rKzRxrCKFcEytCt6szwd2GHnGPRCBLAEAoSVDefSNk4Bt"
// ;     const ALICE_BOB_PATH: &str = "m/0'";

//     #[test]
//     fn test_extract_satisfaction() {
//         const ALICE_SIGNED_PSBT: &str =
// "cHNidP8BAFMBAAAAAZb0njwT2wRS3AumaaP3yb7T4MxOePpSWih4Nq+jWChMAQAAAAD/////
// Af4lAAAAAAAAF6kUXv2Fn+YemPP4PUpNR1ZbU16/
// eRCHAAAAAAABASuJJgAAAAAAACIAIERw5kTLo9DUH9QDJSClHQwPpC7VGJ+ZMDpa8U+2fzcYIgIDeAtjYQk/
// Vfu4db2+68hyMKjc38+kWl5sP5QH8L42ZstHMEQCIBj0jLjUeVYXNQ6cqB+gbtvuKMjV54wSgWlm1cfcgpHVAiBa3DtC9l/
// 1Mt4IDCvR7mmwQd3eAP/m5++81euhJNSrgQEBBUdSIQN4C2NhCT9V+7h1vb7ryHIwqNzfz6RaXmw/lAfwvjZmyyEC+GE/
// y+LptI8xmiR6sOe998IGzybox0Qfz4+BQl1nmYhSriIGAvhhP8vi6bSPMZokerDnvffCBs8m6MdEH8+PgUJdZ5mIDBwu7j4AAACAAAAAACIGA3gLY2EJP1X7uHW9vuvIcjCo3N/
// PpFpebD+UB/C+NmbLDMkRfC4AAACAAAAAAAAA";         const BOB_SIGNED_PSBT: &str =
// "cHNidP8BAFMBAAAAAZb0njwT2wRS3AumaaP3yb7T4MxOePpSWih4Nq+jWChMAQAAAAD/////
// Af4lAAAAAAAAF6kUXv2Fn+YemPP4PUpNR1ZbU16/
// eRCHAAAAAAABASuJJgAAAAAAACIAIERw5kTLo9DUH9QDJSClHQwPpC7VGJ+ZMDpa8U+2fzcYIgIC+GE/
// y+LptI8xmiR6sOe998IGzybox0Qfz4+BQl1nmYhIMEUCIQD5zDtM5MwklurwJ5aW76RsO36Iqyu+6uMdVlhL6ws2GQIgesAiz4dbKS7UmhDsC/
// c1ezu0o6hp00UUtsCMfUZ4anYBAQVHUiEDeAtjYQk/
// Vfu4db2+68hyMKjc38+kWl5sP5QH8L42ZsshAvhhP8vi6bSPMZokerDnvffCBs8m6MdEH8+PgUJdZ5mIUq4iBgL4YT/
// L4um0jzGaJHqw5733wgbPJujHRB/Pj4FCXWeZiAwcLu4+AAAAgAAAAAAiBgN4C2NhCT9V+7h1vb7ryHIwqNzfz6RaXmw/
// lAfwvjZmywzJEXwuAAAAgAAAAAAAAA==";         const ALICE_BOB_SIGNED_PSBT: &str =
// "cHNidP8BAFMBAAAAAZb0njwT2wRS3AumaaP3yb7T4MxOePpSWih4Nq+jWChMAQAAAAD/////
// Af4lAAAAAAAAF6kUXv2Fn+YemPP4PUpNR1ZbU16/
// eRCHAAAAAAABASuJJgAAAAAAACIAIERw5kTLo9DUH9QDJSClHQwPpC7VGJ+ZMDpa8U+2fzcYIgIC+GE/
// y+LptI8xmiR6sOe998IGzybox0Qfz4+BQl1nmYhIMEUCIQD5zDtM5MwklurwJ5aW76RsO36Iqyu+6uMdVlhL6ws2GQIgesAiz4dbKS7UmhDsC/
// c1ezu0o6hp00UUtsCMfUZ4anYBIgIDeAtjYQk/
// Vfu4db2+68hyMKjc38+kWl5sP5QH8L42ZstHMEQCIBj0jLjUeVYXNQ6cqB+gbtvuKMjV54wSgWlm1cfcgpHVAiBa3DtC9l/
// 1Mt4IDCvR7mmwQd3eAP/m5++81euhJNSrgQEBBUdSIQN4C2NhCT9V+7h1vb7ryHIwqNzfz6RaXmw/lAfwvjZmyyEC+GE/
// y+LptI8xmiR6sOe998IGzybox0Qfz4+BQl1nmYhSriIGAvhhP8vi6bSPMZokerDnvffCBs8m6MdEH8+PgUJdZ5mIDBwu7j4AAACAAAAAACIGA3gLY2EJP1X7uHW9vuvIcjCo3N/
// PpFpebD+UB/
// C+NmbLDMkRfC4AAACAAAAAAAEHAAEI2wQARzBEAiAY9Iy41HlWFzUOnKgfoG7b7ijI1eeMEoFpZtXH3IKR1QIgWtw7QvZf9TLeCAwr0e5psEHd3gD/
// 5ufvvNXroSTUq4EBSDBFAiEA+cw7TOTMJJbq8CeWlu+kbDt+iKsrvurjHVZYS+sLNhkCIHrAIs+HWyku1JoQ7Av3NXs7tKOoadNFFLbAjH1GeGp2AUdSIQN4C2NhCT9V+7h1vb7ryHIwqNzfz6RaXmw/
...
//         assert_matches!(&policy_alice_psbt.satisfaction, Satisfaction::Partial { n, m, items, ..
// } if n == &2              && m == &2
//              && items == &vec![0]
//         );
...
//         assert_matches!(&policy_bob_psbt.satisfaction, Satisfaction::Partial { n, m, items, .. }
// if n == &2              && m == &2
//              && items == &vec![1]
//         );
...
//             .unwrap()
//             .unwrap();
//         assert_matches!(&policy_alice_bob_psbt.satisfaction, Satisfaction::PartialComplete { n,
// m, items, .. } if n == &2              && m == &2
//              && items == &vec![0, 1]
//         );
//     }
...
// PpFpebD+UB/C+NmbLDMkRfC4AAACAAAAAAAAA";         const PSBT_POLICY_CONSIDER_TIMELOCK_EXPIRED:
// &str = "cHNidP8BAFMCAAAAAdld52uJFGT7Yde0YZdSVh2vL020Zm2exadH5R4GSNScAAAAAAACAAAAATrcAAAAAAAAF6kUXv2Fn+YemPP4PUpNR1ZbU16/
// eRCHAAAAAAABASvI3AAAAAAAACIAILhzvvcBzw/Zfnc9ispRK0PCahxn1F6RHXTZAmw5tqNPAQVSdmNSsmlofCEDeAtjYQk/
// Vfu4db2+68hyMKjc38+kWl5sP5QH8L42Zsusk3whAvhhP8vi6bSPMZokerDnvffCBs8m6MdEH8+PgUJdZ5mIrJNShyIGAvhhP8vi6bSPMZokerDnvffCBs8m6MdEH8+PgUJdZ5mIDBwu7j4AAACAAAAAACIGA3gLY2EJP1X7uHW9vuvIcjCo3N/
// PpFpebD+UB/C+NmbLDMkRfC4AAACAAAAAAAAA";         const
// PSBT_POLICY_CONSIDER_TIMELOCK_EXPIRED_SIGNED: &str
// ="cHNidP8BAFMCAAAAAdld52uJFGT7Yde0YZdSVh2vL020Zm2exadH5R4GSNScAAAAAAACAAAAATrcAAAAAAAAF6kUXv2Fn+YemPP4PUpNR1ZbU16/
// eRCHAAAAAAABASvI3AAAAAAAACIAILhzvvcBzw/Zfnc9ispRK0PCahxn1F6RHXTZAmw5tqNPIgIDeAtjYQk/
// Vfu4db2+68hyMKjc38+kWl5sP5QH8L42ZstIMEUCIQCtZxNm6H3Ux3pnc64DSpgohMdBj+57xhFHcURYt2BpPAIgG3OnI7bcj/
// 3GtWX1HHyYGSI7QGa/zq5YnsmK1Cw29NABAQVSdmNSsmlofCEDeAtjYQk/
// Vfu4db2+68hyMKjc38+kWl5sP5QH8L42Zsusk3whAvhhP8vi6bSPMZokerDnvffCBs8m6MdEH8+PgUJdZ5mIrJNShyIGAvhhP8vi6bSPMZokerDnvffCBs8m6MdEH8+PgUJdZ5mIDBwu7j4AAACAAAAAACIGA3gLY2EJP1X7uHW9vuvIcjCo3N/
// PpFpebD+UB/C+NmbLDMkRfC4AAACAAAAAAAEHAAEIoAQASDBFAiEArWcTZuh91Md6Z3OuA0qYKITHQY/
// ue8YRR3FEWLdgaTwCIBtzpyO23I/
// 9xrVl9Rx8mBkiO0Bmv86uWJ7JitQsNvTQAQEBUnZjUrJpaHwhA3gLY2EJP1X7uHW9vuvIcjCo3N/PpFpebD+UB/
// C+NmbLrJN8IQL4YT/L4um0jzGaJHqw5733wgbPJujHRB/Pj4FCXWeZiKyTUocAAA==";
...
//             .unwrap()
//             .unwrap();
//         assert_matches!(&policy.satisfaction, Satisfaction::Partial { n, m, items, .. } if n ==
// &3              && m == &2
//              && items.is_empty()
//         );
...
//             .unwrap()
//             .unwrap();
//         assert_matches!(&policy_expired.satisfaction, Satisfaction::Partial { n, m, items, .. }
// if n == &3              && m == &2
//              && items == &vec![0]
//         );
...
//             .unwrap()
//             .unwrap();
//         assert_matches!(&policy_expired_signed.satisfaction, Satisfaction::PartialComplete { n,
// m, items, .. } if n == &3              && m == &2
//              && items == &vec![0, 1]
//         );
...
//         let policy = wallet_desc.extract_policy(&signers_container, BuildSatisfaction::None,
// &secp);         assert!(policy.is_ok());
//     }
...
//         const UNSIGNED_PSBT: &str =
// "cHNidP8BAFMBAAAAAUKgMCqtGLSiGYhsTols2UJ/VQQgQi/SXO38uXs2SahdAQAAAAD/////
// ARyWmAAAAAAAF6kU4R3W8CnGzZcSsaovTYu0X8vHt3WHAAAAAAABASuAlpgAAAAAACJRIEiEBFjbZa1xdjLfFjrKzuC1F1LeRyI/
// gL6IuGKNmUuSIRYnkGTDxwXMHP32fkDFoGJY28trxbkkVgR2z7jZa2pOJA0AyRF8LgAAAIADAAAAARcgJ5Bkw8cFzBz99n5AxaBiWNvLa8W5JFYEds+42WtqTiQAAA=="
// ;         const SIGNED_PSBT: &str =
// "cHNidP8BAFMBAAAAAUKgMCqtGLSiGYhsTols2UJ/VQQgQi/SXO38uXs2SahdAQAAAAD/////
// ARyWmAAAAAAAF6kU4R3W8CnGzZcSsaovTYu0X8vHt3WHAAAAAAABASuAlpgAAAAAACJRIEiEBFjbZa1xdjLfFjrKzuC1F1LeRyI/
// gL6IuGKNmUuSARNAIsRvARpRxuyQosVA7guRQT9vXr+S25W2tnP2xOGBsSgq7A4RL8yrbvwDmNlWw9R0Nc/6t+IsyCyy7dD/
// lbUGgyEWJ5Bkw8cFzBz99n5AxaBiWNvLa8W5JFYEds+42WtqTiQNAMkRfC4AAACAAwAAAAEXICeQZMPHBcwc/
// fZ+QMWgYljby2vFuSRWBHbPuNlrak4kAAA=";
...
//         const UNSIGNED_PSBT: &str =
// "cHNidP8BAFMBAAAAAWZalxaErOL7P3WPIUc8DsjgE68S+ww+uqiqEI2SAwlPAAAAAAD/////
...
// +7VHz6F+nsz8JVxnzWzkgToYjIHhGyuexxtRVKevRx4YwWR/W0r7LPHt6oS6DLlzyuYQarMAhFnhGyuexxtRVKevRx4YwWR/
// W0r7LPHt6oS6DLlzyuYQaLQH2onWFc3UR6I9ZhuHVeJCi5LNAf4APVd7mHn4BhdViHRwu7j4AAACAAgAAACEWgiPY+kcolS1Hp0niOK/
// +7VHz6F+nsz8JVxnzWzkgToYNAMkRfC4AAACAAgAAAAEXIIIj2PpHKJUtR6dJ4jiv/u1R8+hfp7M/
// CVcZ81s5IE6GARgg9qJ1hXN1EeiPWYbh1XiQouSzQH+AD1Xe5h5+AYXVYh0AAA==";         const SIGNED_PSBT:
// &str = "cHNidP8BAFMBAAAAAWZalxaErOL7P3WPIUc8DsjgE68S+ww+uqiqEI2SAwlPAAAAAAD/////
// AQiWmAAAAAAAF6kU4R3W8CnGzZcSsaovTYu0X8vHt3WHAAAAAAABASuAlpgAAAAAACJRINa6bLPZwp3/
// CYWoxyI3mLYcSC5f9LInAMUng94nspa2AQcAAQhCAUALcP9w/
// +Ddly9DWdhHTnQ9uCDWLPZjR6vKbKePswW2Ee6W5KNfrklus/8z98n7BQ1U4vADHk0FbadeeL8rrbHlARNAC3D/cP/
// g3ZcvQ1nYR050Pbgg1iz2Y0erymynj7MFthHuluSjX65JbrP/M/fJ+wUNVOLwAx5NBW2nXni/
// K62x5UEUeEbK57HG1FUp69HHhjBZH9bSvss8e3qhLoMuXPK5hBr2onWFc3UR6I9ZhuHVeJCi5LNAf4APVd7mHn4BhdViHUAXNmWieJ80Fs+PMa2C186YOBPZbYG/
// ieEUkagMwzJ788SoCucNdp5wnxfpuJVygFhglDrXGzujFtC82PrMohwuIhXBgiPY+kcolS1Hp0niOK/
// +7VHz6F+nsz8JVxnzWzkgToYjIHhGyuexxtRVKevRx4YwWR/W0r7LPHt6oS6DLlzyuYQarMAhFnhGyuexxtRVKevRx4YwWR/
// W0r7LPHt6oS6DLlzyuYQaLQH2onWFc3UR6I9ZhuHVeJCi5LNAf4APVd7mHn4BhdViHRwu7j4AAACAAgAAACEWgiPY+kcolS1Hp0niOK/
// +7VHz6F+nsz8JVxnzWzkgToYNAMkRfC4AAACAAgAAAAEXIIIj2PpHKJUtR6dJ4jiv/u1R8+hfp7M/
// CVcZ81s5IE6GARgg9qJ1hXN1EeiPWYbh1XiQouSzQH+AD1Xe5h5+AYXVYh0AAA==";
...
//         assert_matches!(policy_unsigned.item, SatisfiableItem::Thresh { ref items, threshold: 1 }
// if items.len() == 2);         assert_matches!(policy_unsigned.satisfaction, Satisfaction::Partial
// { n: 2, m: 1, items, .. } if items.is_empty());

//         assert_matches!(policy_signed.item, SatisfiableItem::Thresh { ref items, threshold: 1 }
// if items.len() == 2);         assert_matches!(policy_signed.satisfaction,
// Satisfaction::PartialComplete { n: 2, m: 1, items, .. } if items == vec![0, 1]);
