    ...
    K: Ord + Clone + fmt::Debug,
{
    ...
    ///
    /// This method returns [`DescriptorError`](crate::descriptor::error::Error) if the provided
    /// descriptor is multipath, contains hardened derivation steps (in the case of public
    /// descriptors) or fails miniscripts sanity checks.
    pub fn new(
        network: Network,
        keychain: K,
    ...
    /// Add a descriptor. Must not be [multipath](miniscript::descriptor::Descriptor::is_multipath).
    /// This method returns [`DescriptorError`](crate::descriptor::error::Error) if the provided
    /// descriptor is multipath, contains hardened derivation steps (in case of public
    /// descriptors) or fails miniscripts sanity checks. It also returns the error when exactly
